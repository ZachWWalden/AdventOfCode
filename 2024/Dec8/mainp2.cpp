	...
	for(int i = 0; i < MAX_FREQS; i++)
	{
		...
		for(int j = 0; antennas[i] ->size() != 0 && j < antennas[i]->size() - 1; j++)
		{
			...
			for(int k = j + 1; k < antennas[i]->size(); k++)
			{
				...
				//we have a pair yay, both points are now antinodes.
				antinodes[p1.key] = p1;
				antinodes[p2.key] = p2;
				//p2 + d(x/y)
				int an1_x = p2.x + dx, an1_y = p2.y + dy;
				while(isInBounds(an1_x, an1_y, data[0].length(), data.size()))
				{
				...
				while(isInBounds(an2_x, an2_y, data[0].length(), data.size()))
				{
