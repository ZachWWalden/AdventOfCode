...
uint64_t validateRange(Range range)
{
	...
	for(uint64_t i = range.start; i <= range.end; i++)
	{
		...
		//brute force ts.
		//One id can only be invalid once. pattern must repeat at least twice rpt_len > str_len / 2 -> in valid.
		//str len
		//rpt len
		bool invalid = false;
		//ensure rpt len divides str len.
		for(int rpt_len = 1; rpt_len <= num.length() >> 1 && !invalid; rpt_len++)
		{
			if((num.length() % rpt_len) == 0)
			{
				//check for eqivalency.
				//calculate num rpts.
				int num_rpts = num.length() / rpt_len;
				bool valid = false;
				//loop through each char in the rpt
				for(int j = 0; j < rpt_len && !valid; j++)
				{
					//compare positional char in each rpt.
					int offset = rpt_len;
					for(int k = 1; k < num_rpts && !valid; k++)
					{
