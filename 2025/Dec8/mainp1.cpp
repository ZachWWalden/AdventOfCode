...
#if DEBUG
#define LEN_CIRCUIT 10
#else
#define LEN_CIRCUIT 1000
#endif
...
bool compareCircuit(const Circuit *a, const Circuit *b);
...
int main(int argc, char* argv[])
{
	...
	// std::cout << "Sort Circuits" << std::endl;
	//sort circuits by size in descending order.
	uint64_t top_3[3] = {0, 0, 0};
	for(Circuit *ct : circuits)
	{
		if(!ct->deleted)
		{
			// std::cout << "Circuit Size = " << ct->points.size() << std::endl;
			if(ct->points.size() > top_3[0])
			{
				top_3[2] = top_3[1];
				top_3[1] = top_3[0];
				top_3[0] = ct->points.size();
			}
			else if(ct->points.size() > top_3[1])
			{
				top_3[2] = top_3[1];
				top_3[1] = ct->points.size();
			}
			else if(ct->points.size() > top_3[2])
				top_3[2] = ct->points.size();
		}
	}
...
bool compareCircuit(const Circuit *a, const Circuit *b)
{
	// std::cout << "Compare Circuits" << std::endl;
	// std::cout << "A addr = " << a << ", B addr = " << b << std::endl;
	if((void*)a <= (void*)0x10000 || (void*)b <= (void*)0x10000)
	{
		std::cout << "Sort func changed addr" << std::endl;
		return true;
	}
	//does not matter
	...
	if(a->deleted == true && b->deleted == false)
		return false;
